...
@Component({
  selector: 'app-product-filtered',
  templateUrl: './product-filtered.component.html',
  styleUrls: ['./product-filtered.component.scss']
})
export class ProductFilteredComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      // Call a function to fetch the filtered products based on the parentId
  ...
  getProducts(id: any){
    this._service.getProducts(0, 10).subscribe((data: Product) => {
      // this.products = data;
      this.productList = data.items.filter((product: any) => product.categoryId === id); 
...
filterProducts(type: string, value: string, e: any) {
  ...
  this._service.multiFilterProducts(0, 10, 'name', 'asc', this.activeFilters)
    .subscribe(
      (response: any) => {
        this.productList = response.items.filter((product: any) => product.categoryId === this.parentId); // No need to filter again, backend handles it.
