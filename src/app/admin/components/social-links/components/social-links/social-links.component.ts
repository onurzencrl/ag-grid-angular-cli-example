...
deleteRow() {
  this.selectedDataSubscription = this.gridEventService.getSelectedData().subscribe(data => {
    ...
    this.selectedData.forEach((item) => {
        this._service.deleteSocialLink(item.id).subscribe(
          response => {
            console.log('SocialLink deleted successfully!', response);
          },
          error => {
...
getSocialLinks(){
  this._service.getSocialLinks(0, 10).subscribe((data: SocialLink) => {
    // this.products = data;
...
constructor(private _service : SocialLinksService ,  private fb: FormBuilder , private gridEventService: OnurTableService) {
  this.gridOptions = {
...
update(data: any) {
  console.log('Received data from SocialLinksComponent:', data);
  data[0].isDeleted = true;
  
  this._service.updateSocialLink(data[0]).subscribe(
    response => {
      console.log('SocialLink updated successfully!', response);
    },
    error => {
...
onSubmit() {
  if (this.productForm.valid) {
    var data : SocialLinkModel = this.productForm.value;
    this._service.createSocialLink(data).subscribe(
      response => {
        console.log('SocialLink created successfully!', response);
        this.getSocialLinks()
