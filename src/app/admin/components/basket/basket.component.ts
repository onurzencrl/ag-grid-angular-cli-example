...
export class BasketComponent implements OnInit, OnDestroy {
  ...
  deleteRow() {
    this.selectedDataSubscription = this.gridEventService.getSelectedData().subscribe((data : any ) => {
      ...
      this.selectedData.forEach((item) => {
        this._service.deleteBasket(item.id).subscribe(
          response => {
  ...
  getBaskets() {
    this._service.getBaskets(0, 10).subscribe((data : any ) => {
  ...
  constructor(private _service: BasketService, private fb: FormBuilder, private gridEventService: OnurTableService) {
    this.gridOptions = {
  ...
  update(data: any) {
    console.log('Received data from BasketComponent:', data);
    data[0].isDeleted = true;
    this._service.updateBasket(data[0]).subscribe(
      response => {
  ...
  onSubmit() {
    if (this.basketForm.valid) {
      var data: BasketModel = this.basketForm.value;
      this._service.createBasket(data).subscribe(
        response => {
