...
export class CommentComponent implements OnInit, OnDestroy {
  ...
  deleteRow() {
    this.selectedDataSubscription = this.gridEventService.getSelectedData().subscribe((data : any ) => {
      ...
      this.selectedData.forEach((item) => {
        this._service.deleteComment(item.id).subscribe(
          response => {
  ...
  getComments() {
    this._service.getComments(0, 10).subscribe((data : any ) => {
  ...
  constructor(private _service: CommentService, private fb: FormBuilder, private gridEventService: OnurTableService) {
    this.gridOptions = {
  ...
  update(data: any) {
    console.log('Received data from CommentComponent:', data);
    data[0].isDeleted = true;
    this._service.updateComment(data[0]).subscribe(
      response => {
  ...
  onSubmit() {
    if (this.commentForm.valid) {
      var data: CommentModel = this.commentForm.value;
      this._service.createComment(data).subscribe(
        response => {
